const TestimonialsCard = () => {
  return (
    <>
      <div className="relative w-full max-w-sm p-8 pt-16 text-center shadow-xl bg-muted md:max-w-md lg:max-w-lg rounded-2xl min-h-90">
        <img
          src="./anh2.png"
          className="absolute object-cover w-20 h-20 -translate-x-1/2 border-4 rounded-full shadow-lg border-muted md:w-24 md:h-24 -top-10 left-1/2"
        />
        <h3 className="text-xl font-bold md:text-2xl">Nguyễn Quốc Huy</h3>
        <p className="text-primary">@nguyenquochuy</p>
        <p className="mt-4 text-muted-foreground">
          Đây là lần đầu mình đặt tour online nhưng trải nghiệm rất tốt. Nhân
          viên tư vấn rõ ràng, tour đúng mô tả, chuyến đi rất đáng nhớ. Mọi thứ
          đều được chuẩn bị kỹ lưỡng, lịch trình hợp lý, không bị phát sinh chi
          phí. Rất đáng tin cậy.
        </p>
      </div>
    </>
  );
};
export const Testimonials = () => {
  return (
    <div className="flex flex-col items-center w-full h-auto gap-10 bg-[rgb(57,56,56)] bg-[url('./topography.svg')] rounded-4xl">
      <div className="flex flex-col items-center justify-center ">
        <p className="mt-20 text-4xl font-bold text-muted">Lạc Việt Travel</p>
        <h1 className="my-5 text-3xl font-bold text-muted">
          Một số đánh giá từ khách hàng của chúng tôi
        </h1>
        <p className="w-3/5 text-center text-medium font-me text-muted">
          Chúng tôi luôn sẵn sàng lắng nghe và hỗ trợ bạn trong việc lựa chọn
          tour, lên kế hoạch hành trình và giải đáp mọi thắc mắc để chuyến đi
          của bạn diễn ra suôn sẻ nhất
        </p>
      </div>
      <div className="grid grid-cols-1 gap-20 ml-10 mr-10 xl:grid-cols-3 my-30">
        <TestimonialsCard />
        <TestimonialsCard />
        <TestimonialsCard />
      </div>
    </div>
  );
};
